...
public class MongoPatientRepository implements PatientRepository {
    ...
    public ObjectId add(Patient patient) {
        Document doc = new Document("name", patient.getName())
    ...
    private Patient documentToPatient(Document doc) {
        ...
        if (birthDateObj instanceof Date) {
            patient.setBirthDate(((Date) birthDateObj).toInstant().atZone(ZoneId.systemDefault()).toLocalDate());
        } else if (birthDateObj instanceof String) {
            patient.setBirthDate(LocalDate.parse((String) birthDateObj));
        }

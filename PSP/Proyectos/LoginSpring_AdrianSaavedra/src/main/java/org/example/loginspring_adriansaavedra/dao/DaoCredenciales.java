...
public class DaoCredenciales {
    ...
    public CredentialEntity authenticateUser(String username, String password) {
        try {
            CredentialEntity credential = entityManager.createQuery("SELECT c FROM CredentialEntity c WHERE c.username = :username", CredentialEntity.class)
            ...
            throw new UserNotFoundException(Constantes.USER_NOT_FOUND + username);
        }
    }

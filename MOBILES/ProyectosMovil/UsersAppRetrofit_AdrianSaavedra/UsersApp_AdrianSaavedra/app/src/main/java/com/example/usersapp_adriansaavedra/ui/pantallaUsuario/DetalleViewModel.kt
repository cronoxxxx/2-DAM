...
import com.example.usersapp_adriansaavedra.domain.usecases.usuarios.*
...
class DetalleViewModel @Inject constructor(
    ...
    val uiState: LiveData<DetalleState> = _uiState
        ...
        viewModelScope.launch {
            when (val result = getUserUseCase(id)) {
                ...
                is NetworkResult.Error -> {
                    _uiState.value = _uiState.value?.copy(
                        aviso = UiEvent.ShowSnackbar(stringProvider.getString(R.string.error_obteniendo_usuario)),
                    )
                }
    ...
    private fun updateUser(id: Int, user: User) {
        viewModelScope.launch {
            when (val result = updateUserUseCase(id, user)) {
                ...
                is NetworkResult.Error -> {
                    _uiState.value = _uiState.value?.copy(
                        aviso = UiEvent.ShowSnackbar(stringProvider.getString(R.string.error_actualizando_usuario)),
                    )
                }
            }
        }

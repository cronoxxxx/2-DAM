...
class UserRepository @Inject constructor(
    ...
    suspend fun insertUser(user: User) = flow {
        ...
        if (remoteResult is NetworkResult.Success) {
            val existingUser = remoteResult.data.find { it.username == user.username }
            if (existingUser != null) {
                val currentUser = existingUser.copy(password = user.password)
                val obtained = userDao.insertUser(currentUser.toUserEntity())
    ...
    suspend fun updateUser(user: User) = flow {
        emit(NetworkResult.Loading())
        val getUser = userDao.getUser(user.id)
        val receivedToUpdate = user.copy(password = getUser.password)
        val obtained = userDao.updateUser(receivedToUpdate.toUserEntity())
        emit(NetworkResult.Success(obtained))
    }.catch { emit(NetworkResult.Error(it.message ?: it.toString())) }
        .flowOn(dispatcher)
